...
def sort_emitters_by_symbol(emitters: list[Emitter]) -> list[Emitter]:
    ...
    ascending order.
    '''
    # copy the emitter objects into a new list so we don't modify the original
    ...
    while i < len(new_list):
        j = i
        while j < len(new_list):
            # compare the symbols of the two adjacent emitters
            # swap them if the component at the front has a larger symbol
            if new_list[i].get_symbol() > new_list[j].get_symbol():
                # swap the positions of the emitters in the list
                temp = new_list[i]
                new_list[i] = new_list[j]
                new_list[j] = temp
            j += 1

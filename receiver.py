...
class Receiver:
    ...
    def absorb_photon(self, photon: Photon, timestamp: int) -> None:
        ...
        #if this is the first photon absorbed, update activation_time & activated
        if self.photons_absorbed == 0:
            self.activation_time = timestamp
            self.activated = True
            
        #absorbs the photon and updates number of photons_absorbed
        photon.got_absorbed()
        self.photons_absorbed += 1
        
        #update receiver's total energy
